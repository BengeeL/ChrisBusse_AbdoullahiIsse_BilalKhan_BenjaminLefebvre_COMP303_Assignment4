...
interface ScheduleDonationProps {
  addDonation: (donation: string) => void;
  bloodGroup: string;
}

export default function ScheduleDonation({ addDonation, bloodGroup }: ScheduleDonationProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addDonation(JSON.stringify(donations));
      setBloodBank('');
      setDate('');
    } catch (error) {
